...
public class EnergyOption extends TempadOption {
    ...
    public void addToolTip(ItemStack stack, Level level, List<Component> components, TooltipFlag flag) {
        if(stack.getItem() instanceof EnergyItem energyItem) {
            components.add(new TranslatableComponent("tooltip.tempad.energy_info", energyItem.getEnergy(stack), energyItem.getMaxEnergy()).withStyle(ChatFormatting.GRAY));
        }
        components.add(new TranslatableComponent("tooltip.tempad.energy_cost", ConfigUtils.getOptionConfig(getType()).getEnergyCost()).withStyle(ChatFormatting.DARK_GRAY));
    }
