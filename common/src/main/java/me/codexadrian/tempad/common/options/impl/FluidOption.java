...
public class FluidOption implements FuelOption {
    ...
    public boolean canTimedoorOpen(Player player, ItemStack stack) {
        ...
        return !fluidStorage.getFluids().get(0).isEmpty() && fluidStorage.getFluids().get(0).getFluidAmount() >= FuelOptionsApi.API.getFuelCost(stack) && fluidStorage.getFluids().get(0).is(Tempad.TEMPAD_LIQUID_FUEL_TAG);
    }
    ...
    public void onTimedoorOpen(Player player, ItemStack stack) {
        ...
        fluid.setAmount(FuelOptionsApi.API.getFuelCost(stack));
