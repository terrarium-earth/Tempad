...
public class ExperiencePointsOption implements FuelOption {
    ...
    public boolean canTimedoorOpen(Player player, ItemStack stack) {
        return player.totalExperience >= FuelOptionsApi.API.getFuelCost(stack);
    }

    @Override
    public void onTimedoorOpen(Player player, ItemStack stack) {
        player.giveExperiencePoints(-FuelOptionsApi.API.getFuelCost(stack));
    }

    @Override
    public void addToolTip(ItemStack stack, Level level, List<Component> components, TooltipFlag flag) {
        components.add(Component.translatable("tooltip.tempad.experience_points_cost", FuelOptionsApi.API.getFuelCost(stack)));
    }
