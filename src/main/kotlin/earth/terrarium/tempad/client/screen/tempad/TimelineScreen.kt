...
class TimelineScreen(menu: ModMenus.TimelineMenu, inv: Inventory, title: Component): AbstractTempadScreen<ModMenus.TimelineMenu>(
    ...
    override fun init() {
        ...
        addRenderableWidget(Widgets.text(trackingText) {
            it.withPosition(localLeft + 133, localTop + 23)
            it.withSize(font.width(teleportButton) + 12, 16)
            it.withColor(Tempad.ORANGE)
        })

        addRenderableWidget(Widgets.text(inv.player.name) {
            it.withPosition(localLeft + 133, localTop + 98)
            it.withSize(61, 17)
            it.withColor(Tempad.ORANGE)
        })
    }
