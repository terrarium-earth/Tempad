...
import earth.terrarium.tempad.common.block.*
...
object TimedoorComponentProvider : IEntityComponentProvider {
    ...
    override fun appendTooltip(tooltip: ITooltip, accessor: EntityAccessor, config: IPluginConfig) {
        ...
        tooltip.add(
            Component.translatable(
                "jade.tempad.pos",
                Component.translatable(timedoorEntity.targetDimension.identifier().toLanguageKey("dimension")),
                "${timedoorEntity.targetPos.x().toInt()}, ${
    ...
    override fun appendTooltip(tooltip: ITooltip, accessor: BlockAccessor, config: IPluginConfig) {
        (accessor.blockEntity as? AbstractMarkerBe)?.let {
            it.owner?.let {
                tooltip.add(
                    Component.translatable(
                        "item.tempad.location_card.created_by",
                        Component.literal(it.name).withStyle(ChatFormatting.AQUA)
            ...
            it.id?.let {
                if (accessor.player.isShiftKeyDown) {
                    tooltip.add(
                        Component.translatable("item.tempad.location_card.id", it.toString())
                            .withStyle(ChatFormatting.DARK_GRAY)
                    )
                } else {
                    tooltip.add(
                        Component.translatable(
                            "misc.tempad.shift_key_info",
                            Component.keybind(Minecraft.getInstance().options.keyShift.name).withStyle(
                                ChatFormatting.GRAY
                            )
                        ).withStyle(ChatFormatting.DARK_GRAY)
                    )
    ...
    override fun appendTooltip(tooltip: ITooltip, accessor: BlockAccessor, config: IPluginConfig) {
        (accessor.blockEntity as? RudimentaryTempadBE)?.let {
            ...
            if (pos is DirectLocation) {
                ...
                tooltip.add(
                    Component.translatable(
                        "item.tempad.location_card.created_by", Component.literal(player.name).withStyle(
                            ChatFormatting.AQUA
                        )
                    ).withStyle(ChatFormatting.GRAY)
                )
                if (!accessor.player.isShiftKeyDown) {
                    ...
                    tooltip.add(
                        Component.translatable("item.tempad.location_card.id", id.toString())
                            .withStyle(ChatFormatting.DARK_GRAY)
                    )
                }
            }
        ...
        (accessor.blockEntity as? MetronomeBe)?.let { blockEntity ->
            if (blockEntity.bootTime > 0) {
                ...
                tooltip.add(
                    ProgressElement(
                        ProgressView(
                            ProgressView.Part.of(progress),
                            Component.translatable("block.tempad.metronome.booting", (progress * 100).toInt())
                                .append("%"),
                            SimpleProgressStyle().also { it.color = Tempad.ORANGE.value },
                            BoxStyle.nestedBox()
                        )
                    )
                )
            ...
            blockEntity.owner?.let {
                tooltip.add(
                    Component.translatable(
                        "item.tempad.location_card.created_by", Component.literal(it.name).withStyle(
                            ChatFormatting.AQUA
                        )
                    ).withStyle(ChatFormatting.GRAY)
                )
                    ...
                    ProgressElement(
                        ProgressView(
                            ...
                            Component.translatable("block.tempad.workstation.installing", (progress * 100).toInt())
                                .append("%"),
                            SimpleProgressStyle().also { it.color = MinecraftColors.DARK_GRAY.value },
                            BoxStyle.nestedBox()
                        )
                    )
                )
            }
            blockEntity.inventory.getResource(0).owner?.let {
                tooltip.add(
                    Component.translatable(
                        "item.tempad.location_card.created_by", Component.literal(it.name).withStyle(
                            ChatFormatting.AQUA
