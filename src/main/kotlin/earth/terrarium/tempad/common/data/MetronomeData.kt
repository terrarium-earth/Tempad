...
class MetronomeData(stored: Map<UUID, Int>, positions: Map<UUID, List<GlobalPos>>) {
    ...
    fun tick() {
        for ((player, amount) in stored) {
            stored[player] = (CommonConfig.Metronome.generationAmount * (if(CommonConfig.Metronome.scaleGeneration) getCount(player) else 1) + amount).coerceAtMost(getCapacity(player))
        }
        sync()
    }
